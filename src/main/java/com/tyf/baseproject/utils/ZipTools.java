package com.tyf.baseproject.utils;
...
//		}
	...
	/**
	  ...
	  * @param encoding 解压编码
	 */
	public static String decompress(String zipPath, String targetPath, String encoding)
		...
		while (files.hasMoreElements()) {
			...
			if(entry.getName().indexOf("\\")!=-1){
				fileName = uuid + entry.getName().substring(entry.getName().indexOf("\\"), entry.getName().length());
			}else{
				fileName = uuid + entry.getName().substring(entry.getName().indexOf("/"), entry.getName().length());
			}			
			//outFile = new File(targetPath + File.separator + fileName);
			outFile = new File(targetPath + "/" + fileName);
			// 如果条目为目录，则跳向下一个
		...
		zipFile.close();
		return targetPath + "/"+uuid;
	}
	
	/**
	  * @Description: 
	  * 	解压文件  bmp项目使用 解压出的路径没有uuid
	  * @param zipPath 被压缩文件，请使用绝对路径
	  * @param targetPath 解压路径，解压后的文件将会放入此目录中，请使用绝对路径
	  * 		默认为压缩文件的路径的父目录为解压路径
	  * @param encoding 解压编码
	 */
	public static void decompressWithOutUUID(String zipPath, String targetPath, String encoding)
			throws FileNotFoundException, ZipException, IOException {
		// 获取解缩文件
		File file = new File(zipPath);
		if (!file.isFile()) {
		...
		while (files.hasMoreElements()) {
			...
			// 实例化解压后文件对象
			if(entry.getName().indexOf("\\")!=-1){
				fileName = entry.getName().substring(entry.getName().indexOf("\\"), entry.getName().length());
			}else{
				fileName = entry.getName().substring(entry.getName().indexOf("/"), entry.getName().length());
			}			
			//outFile = new File(targetPath + File.separator + fileName);
...
//		sourcePaths.add("E:\\Program Files\\feiq\\Recv Files\\test");
		compress("F:/test/sct/", "F:/test/zip/aaaa.zip", encoding, "aaaa");
//		compress(sourcePaths, "E:\\Program Files\\feiq\\Recv Files\\bbbb.chp", encoding, "aaaa");
//		decompressWithOutUUID("F:/test/zip/aaaa.bmp", "F:/test/unzip/", encoding);
